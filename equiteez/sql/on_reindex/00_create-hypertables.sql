-- Create hypertable for user_token_transfer
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_token_transfer_pkey') THEN
        ALTER TABLE user_token_transfer DROP CONSTRAINT user_token_transfer_pkey;
    END IF;
    ALTER TABLE user_token_transfer ADD PRIMARY KEY (id, timestamp);
END $$;

SELECT create_hypertable('user_token_transfer', 'timestamp', 
