...
async def origination(
    ...
    price_model = (
        models.PriceModel.FIXED
        if dodo_mav_origination.storage.guidePriceConfig.priceModel == "fixed"
        else models.PriceModel.DYNAMIC
    )
    appraisal_price = dodo_mav_origination.storage.guidePriceConfig.appraisalPrice
    fixed_price_percent = (
        dodo_mav_origination.storage.guidePriceConfig.fixedPricePercent
    )
    orderbook_price_percent = (
        dodo_mav_origination.storage.guidePriceConfig.orderbookPricePercent
    )
    ...
    quote_lp_token_address = (
        dodo_mav_origination.storage.quoteLpToken.tokenContractAddress
    )
    base_lp_token_address = (
        dodo_mav_origination.storage.baseLpToken.tokenContractAddress
    )
    ...
    # Get the orderbook
    orderbook = await models.Orderbook.get_or_none(address=rwa_orderbook_address)
    if not orderbook:
        return
    ...
    quote_token = await register_token(ctx=ctx, address=quote_token_address)
    base_token = await register_token(ctx=ctx, address=base_token_address)
    quote_lp_token = await register_token(ctx=ctx, address=quote_lp_token_address)
    base_lp_token = await register_token(ctx=ctx, address=base_lp_token_address)
    ...
    for entrypoint in pause_ledger:
        ...
        entrypoint_status = models.DodoMavEntrypointStatus(
            contract=dodo_mav, entrypoint=entrypoint, paused=paused
        )
