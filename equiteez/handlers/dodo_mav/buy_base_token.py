...
from equiteez.types.base_token.tezos_parameters.transfer import (
    TransferParameter as BaseTokenTransferParameter,
)
from equiteez.types.base_token.tezos_parameters.transfer import (
    TransferParameter as MaintainerFeeParameter,
)
from equiteez.types.base_token.tezos_storage import BaseTokenStorage
from equiteez.types.dodo_mav.tezos_parameters.buy_base_token import (
    BuyBaseTokenParameter,
)
from equiteez.types.dodo_mav.tezos_storage import DodoMavStorage
from equiteez.types.quote_token.tezos_parameters.transfer import (
    TransferParameter as QuoteTokenTransferParameter,
)
...
async def buy_base_token(
    ...
    quote_token_transfer: TezosTransaction[
        QuoteTokenTransferParameter, QuoteTokenStorage
    ],
    maintainer_fee: TezosTransaction[MaintainerFeeParameter, BaseTokenStorage] | None,
) -> None:
    try:
        # Fetch operation info
        address = buy_base_token.data.target_address
        trader_address = buy_base_token.data.sender_address
        timestamp = buy_base_token.data.timestamp
        ...
        level = buy_base_token.data.level
        ...
        # Save history data
        trader, _ = await models.EquiteezUser.get_or_create(address=trader_address)
        await trader.save()
