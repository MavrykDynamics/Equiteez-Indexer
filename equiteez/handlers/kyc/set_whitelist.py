...
from equiteez.types.kyc.tezos_parameters.set_whitelist import (
    SetWhitelistParameter,
    SetWhitelistAction as addToWhitelist,
)
...
async def set_whitelist(
    ...
    # Update record
    if type(set_whitelist_action) is addToWhitelist:
        ...
        for whitelist_address in whitelist_addresses:
            user, _ = await models.EquiteezUser.get_or_create(address=whitelist_address)
            await user.save()
            await models.KycWhitelisted.filter(kyc=kyc, user=user).delete()
