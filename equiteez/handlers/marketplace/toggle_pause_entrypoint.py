...
from equiteez.types.marketplace.tezos_parameters.toggle_pause_entrypoint import (
...
async def toggle_pause_entrypoint(
    ...
    toggle_pause_entrypoint: TezosTransaction[
        TogglePauseEntrypointParameter, MarketplaceStorage
    ],
) -> None:
    # Fetch operation info
    address = toggle_pause_entrypoint.data.target_address
    create_listing_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.createListingIsPaused
    )
    edit_listing_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.editListingIsPaused
    )
    remove_listing_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.removeListingIsPaused
    )
    purchase_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.purchaseIsPaused
    )
    offer_is_paused = toggle_pause_entrypoint.storage.breakGlassConfig.offerIsPaused
    accept_offer_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.acceptOfferIsPaused
    )
    remove_offer_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.removeOfferIsPaused
    )
    set_currency_is_paused = (
        toggle_pause_entrypoint.storage.breakGlassConfig.setCurrencyIsPaused
    )
