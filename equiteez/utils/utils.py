...
async def get_token_standard(ctx, address):
    ...
    if address == NATIVE_MAV_ADDRESS:
        ...
        try:
            datasource = ctx.get_tezos_tzkt_datasource(name="mvkt")
...
async def get_contract_token_metadata(ctx, address, token_id="0"):
    ...
    try:
        ...
        if not token_metadata:
            ...
            metadata_datasource = ctx.get_tzip_metadata_datasource(
...
async def persist_lambda(contract_class, lambda_contract_class, set_lambda):
    ...
    # Save / Update record
    contract = await contract_class.get(address=contract_address)
